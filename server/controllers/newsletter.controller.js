...
const newsletterController = {
  ...
  async subscribe(req, res) {
    try {
      ...
      logger.error("Erreur d'abonnement à la newsletter:", error);
      res.status(500).json({
        success: false,
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          logger.info(`Newsletter envoyée avec succès à ${sentCount} abonnés.`);
        } catch (error) {
  ...
  async changeSubscriberStatus(req, res) {
    try {
      ...
      logger.error("Erreur de changement de statut:", error);
      res.status(500).json({
        success: false,
